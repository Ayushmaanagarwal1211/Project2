import React, { useState, useEffect } from "react";
import "./App.css";

const ImageComponent = ({ url, ready, error, index }) => {
  const [status, setStatus] = useState(ready ? "ready" : error ? "error" : "loading");
  const [retryCount, setRetryCount] = useState(0);

  useEffect(() => {
    let retries = 0;
    const retryImage = () => {
      if (retries < 3) {
        retries++;
        setRetryCount(retries);
        setStatus("loading");
        setTimeout(() => {
          // Simulate loading image
          const isLoaded = Math.random() > 0.5; // Random success/failure
          setStatus(isLoaded ? "ready" : "error");
        }, 5000);
      }
    };

    if (status === "loading") {
      retryImage();
    }
  }, [status]);

  const renderContent = () => {
    if (status === "loading") {
      return <div className="spinner" />;
    }
    if (status === "error") {
      return <div className="error-icon" title={`Image ${index + 1}: Failed after ${retryCount} retries`} />;
    }
    if (status === "ready") {
      return <img src={url} alt={`Image ${index + 1}`} />;
    }
  };

  return (
    <div className="image-container">
      {renderContent()}
    </div>
  );
};

const App = ({ name, count }) => {
  let images =  [
    { url: "data:image/png;base64,", ready: true, error: false },
    { url: "https://example2.com", ready: false, error: true },
    { url: "https://example3.com", ready: false, error: true },
    { url: "https://example4.com", ready: false, error: true }
  ];
  const hasError = images.some((image) => image.error);

  return (
    <div className="main-container">
      <div className="header">
        <h2>{name}</h2>
        <p>{count}+ offline centers</p>
      </div>
      {hasError && <div className="large-error-icon" title="One or more images failed to load" />}
      <div className="images-grid">
        {images.map((image, index) => (
          <ImageComponent key={index} {...image} index={index} />
        ))}
        {images.length < 4 &&
          Array.from({ length: 4 - images.length }).map((_, idx) => (
            <div key={idx} className="placeholder" title="Empty placeholder" />
          ))}
      </div>
    </div>
  );
};

export default App;
